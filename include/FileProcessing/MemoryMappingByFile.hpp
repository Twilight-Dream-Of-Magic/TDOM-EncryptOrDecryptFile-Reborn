...
namespace MemoryObjectConfrontationDiskFileData
{
	...
	*/
	...
	template <typename MemoryMapType>
	requires TemplateConcept_MemoryMap<MemoryMapType>
	std::tuple<bool, MemoryMapType, std::error_code> MMMO_TryAssociateFile_ToPack( const std::filesystem::path& file_path_name, MemoryMapType* memory_map_pointer );

	template <typename MemoryMapType>
	requires TemplateConcept_MemoryMap<MemoryMapType>
	bool MMMO_FromUnpack( std::tuple<bool, MemoryMapType, std::error_code>& associated_data, MemoryMapType& default_memory_map_object, std::error_code& default_error_code_object);

	template <typename MemoryMapType_ReadAndWrite>
	requires TemplateConcept_MemoryMap_ReadAndWrite<MemoryMapType_ReadAndWrite>
	void MMMO_TrySyncDiskFile( MemoryMapType_ReadAndWrite& mapped, std::error_code& object_error_code );
	...
	std::tuple<bool, MemoryMapType, std::error_code> MMMO_TryAssociateFile_ToPack( const std::filesystem::path& file_path_name, MemoryMapType* memory_map_pointer )
	{
		...
		if constexpr( std::same_as<std::remove_reference_t<decltype(memory_map_reference)>, mio::mmap_sink> || std::same_as<std::remove_reference_t<decltype(memory_map_reference)>, mio::ummap_sink> )
		{
			...
			if ( std::filesystem::exists( file_path_name ) )
			{
				...
				if(!file_stream_object.is_open())
					object_error_code = std::move( std::make_error_code( std::errc::text_file_busy ) );
				else
				{
					//立即关闭文件
					//Close the file now
		...
		memory_map_reference.map( this_file_path_name.string(), 0, mio::map_entire_file, object_error_code );
		...
		if ( !memory_map_reference.is_open() )
		{
			...
			const int error_code_number = object_error_code.value();
		...
		else
		{
			...
			else
			{
				...
				std::cerr << "The memory mapped objects cannot be associated with files.\nThe file path is. "
							<< "[" << CommonToolkit::from_u8string(file_path_name.u8string()) << "]" << std::endl;
				object_error_code = std::move( std::make_error_code( std::errc::io_error ) );
				UnmappingMemoryMapObject( memory_map_reference );
				return std::make_tuple( std::move( false ), std::move( memory_map_reference ), std::move( object_error_code ) );
			}
		}
	...
	template <typename MemoryMapType>
	requires TemplateConcept_MemoryMap<MemoryMapType>
	bool MMMO_FromUnpack( std::tuple<bool, MemoryMapType, std::error_code>& associated_data, MemoryMapType& default_memory_map_object)
	{
		std::error_code error_code_object;
		return MMMO_FromUnpack(associated_data, default_memory_map_object, error_code_object);
	}
	...
	bool MMMO_FromUnpack( std::tuple<bool, MemoryMapType, std::error_code>& associated_data, MemoryMapType& default_memory_map_object, std::error_code& default_error_code_object)
	{
		...
		auto& [associated_mmap_data_package_status, memory_map_object, error_code_object] = associated_data;
		...
		{
			default_error_code_object = std::move( error_code_object );
			return associated_mmap_data_package_status;
		}
		...
		{
			default_error_code_object = std::move( std::get<std::error_code>( associated_data ) );
			return associated_mmap_data_package_status;
		}
	...
	template <typename MemoryMapType_ReadAndWrite>
	requires TemplateConcept_MemoryMap_ReadAndWrite<MemoryMapType_ReadAndWrite>
	void MMMO_TrySyncDiskFile( MemoryMapType_ReadAndWrite& mapped_object, std::error_code& object_error_code )
	{
		...
		else
		{
			...
			object_error_code = std::move( std::make_error_code( std::errc::invalid_argument ) );
		}
	}
	...
	auto mapped_object = MMMO_FromUnpack(associated_mmap_data_package, associated_mmap_data_unpackage_status, error_code_object);
	std::error_code error_code_object2 = MMMO_TrySyncDiskFile(mapped_object, error_code_object);
