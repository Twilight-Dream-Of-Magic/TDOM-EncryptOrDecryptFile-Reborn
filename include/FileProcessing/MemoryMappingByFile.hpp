...
namespace MemoryObjectConfrontationDiskFileData
{
	namespace MIO_LibraryHelper
	{
		...
		*/
		...
		template <typename MemoryMapType>
		requires TemplateConcept_MemoryMap<MemoryMapType>
		bool MappedMemoryMapObject_FromUnpack( std::tuple<bool, MemoryMapType, std::error_code>& associated_data, MemoryMapType& default_memory_map_object, std::error_code& default_error_code_object);

		template <typename MemoryMapType_ReadAndWrite>
		requires TemplateConcept_MemoryMap_ReadAndWrite<MemoryMapType_ReadAndWrite>
		void NeedSyncDiskFile_ByManagedDataIsChanged_WithMappedObject( MemoryMapType_ReadAndWrite& mapped, std::error_code& object_error_code );
		...
		std::tuple<bool, MemoryMapType, std::error_code> MappingMemoryMapObject_TryAssociateFile_ToPack( const std::filesystem::path& file_path_name, MemoryMapType* memory_map_pointer )
		{
			...
			if ( std::is_same_v<std::remove_reference_t<decltype(memory_map_reference)>, mio::mmap_sink> || std::is_same_v<std::remove_reference_t<decltype(memory_map_reference)>, mio::ummap_sink> )
			{
				...
				if ( std::filesystem::exists( file_path_name ) )
				{
					...
					_file_path_name += CommonToolkit::from_u8string(u8string_extension_name);
				}
			...
			if ( !memory_map_reference.is_open() )
			{
				...
				error_code_value = CatchErrorCode( object_error_code );
		...
		template <typename MemoryMapType>
		requires TemplateConcept_MemoryMap<MemoryMapType>
		bool MappedMemoryMapObject_FromUnpack( std::tuple<bool, MemoryMapType, std::error_code>& associated_data, MemoryMapType& default_memory_map_object)
		{
			std::error_code error_code_object;
			return MappedMemoryMapObject_FromUnpack(associated_data, default_memory_map_object, error_code_object);
		}
		...
		template <typename MemoryMapType_ReadAndWrite>
		requires TemplateConcept_MemoryMap_ReadAndWrite<MemoryMapType_ReadAndWrite>
		void NeedSyncDiskFile_ByManagedDataIsChanged_WithMappedObject( MemoryMapType_ReadAndWrite& mapped_object, std::error_code& object_error_code )
		{
