		...
		*/
		...
		)
		{
			...
			if constexpr(false)
			{
				...
				PRNE.InitialBySeed<std::uint32_t, std::vector<std::uint32_t>::iterator>( RandomNumberSeedSequence.begin(), RandomNumberSeedSequence.end(), true );
			}
			else
			{
				PRNE.InitialBySeed<std::uint32_t, std::vector<std::uint32_t>::iterator>( PasswordStreamWords.begin(), PasswordStreamWords.end(), true );
			}
