...
namespace Cryptograph::CommonModule
{
	...
	*/
	...
	//多重密码学的算法
	//Algorithms for multiple cryptography
	...
	void ConvertingInputDataAndTransmission( std::unique_ptr<FileDataCrypticModuleAdapter>& FDCM_Adapter_Pointer, std::deque<std::vector<char>>* pointerWithFileDataBlocks )
	{
		if ( FDCM_Adapter_Pointer != nullptr )
		{
			...
			std::deque<std::vector<std::byte>> Datas = AssociatedObjects.ToBytes( *pointerWithFileDataBlocks );

			while ( Datas.size() != 0 )
			{
				AssociatedObjects.FileDataBytes.push_back( std::move( Datas.front() ) );
				Datas.pop_front();
			}
	...
	void ConvertingOutputDataAndTransmission( std::unique_ptr<FileDataCrypticModuleAdapter>& FDCM_Adapter_Pointer, std::deque<std::vector<std::byte>>* pointerWithFileDataBlocks )
	{
		...
		if ( FDCM_Adapter_Pointer != nullptr )
		{
			...
			std::deque<std::vector<char>> Datas = AssociatedObjects.FromBytes( *pointerWithFileDataBlocks );

			while ( Datas.size() != 0 )
			{
				AssociatedObjects.FileDataCharacters.push_back( std::move( Datas.front() ) );
				Datas.pop_front();
			}
