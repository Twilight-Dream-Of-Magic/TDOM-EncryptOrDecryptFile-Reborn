		...
		namespace ImplementationDetails
		{
			...
			public:
				...
				*/
				inline std::uint32_t Word32Bit_KeyWithStreamCipherFunction(std::span<const std::uint32_t> RandomWordDataMaterial)
				{
					...
					{
						volatile std::uint64_t RandomNumber = NLFSR_Object.unpredictable_bits(RandomWordDataMaterial[0] & 1, 64) ^ LFSR_Object();
			...
			private:
				...
				void OPC_MatrixTransformation()
				{
					...
					//RandomNumberMatrixC = RandomNumberMatrixB ⊙ RandomNumberMatrixA
					//RandomNumberMatrixC = RandomNumberMatrixC mod LargePrimeNumbers
					for(std::size_t MatrixRow = 0; MatrixRow < GeneratedRoundSubkeyMatrix.rows() && MatrixRow < TransformedSubkeyMatrix.rows(); ++MatrixRow)
					{
						...
						{
							const std::uint64_t IntegerA = RandomQuadWordMatrix(MatrixRow, MatrixColumn);
							const std::uint64_t IntegerB = TransformedSubkeyMatrix(MatrixRow, MatrixColumn);

							std::uint64_t& MatrixValue = GeneratedRoundSubkeyMatrix(MatrixRow, MatrixColumn);

							MatrixValue = ~( IntegerA ^ IntegerB );
						}
					}
			...
			public:
				...
				void GenerationRoundSubkeys()
				{
					...
					for(const auto& MatrixRows : GeneratedRoundSubkeyMatrix.rowwise())
					{
						for(const auto& RoundSubkeyMatrixValue : MatrixRows )
						{
					...
					*/
					for(std::size_t Index = 0; Index < RoundSubkeyVectorSpan.size(); Index += 32)
					{
						...
						VectorA, VectorB = Split(StateMatrixY)
						MatrixA, MatrixB = Split(StateMatrixX)
						StateMatrixY = BinaryDiffusion(MatrixA, MatrixB, VectorA, VectorB)
							Vectorα = BinaryMultiplication(VectorA, MatrixA)
							Vectorβ = BinaryMultiplication(VectorB, MatrixB)
							Vectorα, Vectorβ ∈ GaloisFiniteField(power(2, 64))
							StateMatrixY = Concat(Vectorα, Vectorβ)
